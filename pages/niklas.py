...
dash.register_page(__name__, path='/niklas', name='Niklas')
...
layout = html.Div([
    dcc.Dropdown(
        id={'page': 'niklas', 'role': 'dropdown'},
        options=[{'label': key, 'value': key} for key in plots.keys()],
        value=list(plots.keys())[0]
    ),
    dcc.Graph(id={'page': 'niklas', 'role': 'plot'})
])

@callback(
    Output({'page': 'niklas', 'role': 'plot'}, 'figure'),
    Input({'page': 'niklas', 'role': 'dropdown'}, 'value')
)
